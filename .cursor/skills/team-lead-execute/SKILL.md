...
## A. Skill Invocation
- This skill is invoked when the user mentions `team-lead-execute` or asks to run the pipeline for a task.
- Treat all user text after the skill mention as `{{ARGS}}` — `<TASK-ID> [--auto-next]`.
- If no task id is present, ask the user which task to run.

## B. User Prompting
On any failure or cap, prompt the user conversationally (Retry / Skip / Abort) in your response text and wait for their reply — never block on raw stdin. Never auto-retry without explicit user choice.

## C. Tool Usage
- Use `StrReplace` for every single status transition (one transition per call).
- Use `Read`, `Glob`, `Grep`, `Shell` (for `git diff` etc.) as needed.

## D. Subagent Spawning
- Spawn pipeline agents with the `Task` tool, e.g. `Task(subagent_type="be-developer", ...)`, `Task(subagent_type="code-reviewer", ...)`, `Task(subagent_type="qa-be", ...)`, `Task(subagent_type="team-lead-check", ...)`.
- Run each agent with `run_in_background: false` so its receipt is available before advancing.
- Pass the stage context preamble as the start of each agent's prompt.
</cursor_skill_adapter>

## Constraints

- Each stage writes **ONE status transition at a time** using `StrReplace`. Never write two status values in a single edit. Advance ONE STAGE AT A TIME — wait for each edit to complete before proceeding to the next stage.
- The preToolUse hook (`.cursor/hooks/task-state-guard.js`) validates every transition automatically. If a transition is invalid, the hook denies the write and returns the reason. Do not attempt to bypass or work around the hook.
...
Take `{{ARGS}}`. Check for the `--auto-next` flag:

- If `{{ARGS}}` contains `--auto-next`: `SET auto_next = true`, strip `--auto-next` from the string before extracting the task ID.
...
**OUTER LOOP** — TLC rejection loop. Repeat while `tlc_cycle < 2`:
  ...
  **INNER LOOP** — QA rejection loop. Repeat while `qa_cycle < 3`:
  ...
  Use `StrReplace` on the task file: change `status: readyForDevelop` to `status: inProgress`.
  (On re-loop when status is already `inProgress`, this edit is a no-op — the hook will deny a same-to-same transition; skip the edit and proceed directly to the Task call if the task is already `inProgress`.)
  ...
  **Stopped check:** Re-read the task file at `<task_path>`. If frontmatter `status` is `stopped`:
    ...
    - Print: `Pipeline stopped by user at Developer.`
    - Stop pipeline (do not advance to the next agent).

  Spawn `Task(subagent_type = developer_agent)`. Include the stage context preamble at the top of the agent prompt, followed by:
  ```
  ...
  - Wait for user response.
    - **Retry**: re-spawn developer_agent (do NOT re-edit status). Restart Developer Stage.
  ...
  Use `StrReplace`: change `status: inProgress` to `status: inReview`.
  (On re-loop when status is already `inReview`, the hook will deny a same-to-same transition; skip the edit and proceed directly to the Task call.)
  ...
  **Stopped check:** Re-read the task file. If frontmatter `status` is `stopped`:
    - Append to task file body: `[pipeline] STOPPED`
    - Print: `Pipeline stopped by user at CodeReview.`
    - Stop pipeline.

  Spawn `Task(subagent_type = "code-reviewer")`. Include the stage context preamble at the top of the agent prompt, followed by:
  ```
  ...
  **REVIEW-BLOCK extraction:** In the code-reviewer return value, find the text between the delimiter lines `---REVIEW-BLOCK-START---` and `---REVIEW-BLOCK-END---` (inclusive of the delimiter lines). If both delimiters are present, extract that block. If either delimiter is absent, use this placeholder block instead:
  ...
  Use `StrReplace` to append the extracted REVIEW-BLOCK to the task file body. Target the last line of the current task file content as `old_string`; set `new_string` = that last line + `"\n\n"` + the extracted block.
  ...
  **Signal extraction** (check in this priority order: APPROVED → CHANGES_REQUESTED → ERROR):
  ...
  - If receipt contains `"CHANGES_REQUESTED"`:
    ...
    - If `cr_cycle >= 3`:
      ...
      - Wait for user response.
        - **Retry**: reset `cr_cycle = 0`, `StrReplace`: change `status: inReview` to `status: inProgress`. Continue INNER LOOP from Developer Stage.
    ...
    - Else (below cap):
      - `StrReplace`: change `status: inReview` to `status: inProgress`. (The hook validates the REVIEW-BLOCK with CHANGES_REQUESTED is present before allowing this transition.)
  ...
  Use `StrReplace`: change `status: inReview` to `status: inTesting`.
  (On re-loop when status is already `inTesting`, the hook will deny a same-to-same transition; skip the edit and proceed directly to the Task call.)
  ...
  **Stopped check:** Re-read the task file. If frontmatter `status` is `stopped`:
    - Append to task file body: `[pipeline] STOPPED`
    - Print: `Pipeline stopped by user at QA.`
    - Stop pipeline.

  Spawn `Task(subagent_type = qa_agent)`. Include the stage context preamble at the top of the agent prompt, followed by:
  ```
  ...
  **Signal extraction** (check in this priority order: PASS → FAIL):
  ...
  - If receipt contains `"FAIL"`:
    ...
    - Else (below cap):
      ...
      - `StrReplace`: change `status: inTesting` to `status: inProgress`. (The hook validates the ## QA Results Status: FAIL annotation written by the qa agent is present before allowing this transition.)
  ...
  **END INNER LOOP**
  ...
  Use `StrReplace`: change `status: inTesting` to `status: forTeamLeadCheck`.
  ...
  **Stopped check:** Re-read the task file. If frontmatter `status` is `stopped`:
    - Append to task file body: `[pipeline] STOPPED`
    - Print: `Pipeline stopped by user at TeamLeadCheck.`
    - Stop pipeline.

  Spawn `Task(subagent_type = "team-lead-check")`. Include the stage context preamble at the top of the agent prompt, followed by:
  ```
  ...
  **Signal extraction** (check in this priority order: APPROVED → REJECTED):
  ...
  - If receipt contains `"APPROVED"`:
    - `StrReplace`: change `status: forTeamLeadCheck` to `status: done`.
  ...
  - If receipt contains `"REJECTED"`:
    ...
    - Else (below cap):
      ...
      - `StrReplace`: change `status: forTeamLeadCheck` to `status: inProgress`. (The hook validates the ## TeamLead Check Status: REJECTED annotation written by the team-lead-check agent is present before allowing this transition.)
      - Reset `qa_cycle = 0`.
      - Reset `cr_cycle = 0`.
      - Continue OUTER LOOP.
...
Run Shell to find all task files in the same epic directory and identify the next candidate:
