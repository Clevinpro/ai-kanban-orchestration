...
## A. Skill Invocation
- This skill is invoked when the user mentions `team-lead-plan` or asks to break a SPEC.md into task files.
- Treat all user text after the skill mention as `{{ARGS}}` — either a path to an existing SPEC.md, or `--new <epic-name>`.
- If no arguments are present, ask the user for the SPEC.md path.
...
## C. Tool Usage
- `Read`, `Write`, `Glob`, `Grep` for file work; `Shell` for timestamps (`date`).
...
If `{{ARGS}}` starts with `--new`, treat the next word as `<epic-name>`.

Write a starter SPEC.md template to `.planning/work/<epic-name>/SPEC.md`. The template must contain exactly these four section headers with placeholder content:
...
Treat `{{ARGS}}` as a file path to an existing SPEC.md. Proceed with STEP 1 below.

---

## STEP 1 — Validate SPEC.md

Read the file at `{{ARGS}}`.
...
**Fallback:** If `## Goal` is empty or absent, use the parent directory name of the SPEC.md path, converted to kebab-case using the same rules.
...
- `started-at`: always `null` on creation — set automatically by the task-timestamps hook when status transitions to `inProgress`
- `completed-at`: always `null` on creation — set automatically by the task-timestamps hook when status transitions to `done`
- `spec`: path to the SPEC.md file passed as `{{ARGS}}` to this invocation (e.g., `.planning/work/<slug>/SPEC.md`); used by team-lead-check agent to locate acceptance criteria
...
Print: `Aborted. No files written.` and stop without writing any files.
