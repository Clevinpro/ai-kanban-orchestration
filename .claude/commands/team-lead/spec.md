...
argument-hint: "\"<free-text description of the epic>\" [--name <epic-name>]"
allowed-tools:
  - Read
  - Write
  - Bash
...
`/team-lead:plan` consumes an existing `SPEC.md`. This command **creates** that
...
The SPEC.md this command writes **must** be directly consumable by
`/team-lead:plan` — i.e. it must contain all four headers that `plan.md`'s
...
- **Input is the idea + the chat.** Use `$ARGUMENTS` as the primary description,
...
- **Single repo per epic.** A SPEC.md targets `be`, `fe`, or `kanban` — never a
  mix. Full-stack ideas split into separate epics (so each gets its own SPEC and
  its own `repo:`-pure tasks). State the chosen repo in the header block. If the
  idea is genuinely full-stack, say so and recommend splitting into two specs.
...
- If `$ARGUMENTS` is empty **and** the chat contains a clear, discussed feature,
...
1. **`$ARGUMENTS`** — the explicit description.
...
- Fill every `<...>` from STEP 1–2 material. Leave **no** literal placeholder
  text in the output.
- `created-at` date: use today's date (`date +%Y-%m-%d` via Bash if needed).
...
**If the user replies with anything else (including pressing Enter without
input):**
